...
abstract class RetornoAbstract
{
    ...
    public function __construct($conteudo)
    {
        ...
        if ($length == 240 || $length == 241) {
            ...
            throw new Exception("Não foi possivel detectar os registros");
        }
        if ($codigo_tipo == '1') {
        ...
        $class = '\App\Http\Controllers\resources\\B' . self::$banco . '\retorno\\' . self::$layout . '\Registro0';

        self::$lines = $lines;
        $this->children[] = new $class($lines[0]);

        $class = '\App\Http\Controllers\resources\\B' . self::$banco . '\retorno\\' . self::$layout . '\Registro9';
